...
public class AppointmentDAO {
    ...
    public boolean isSlotAvailable(int doctorId, String date, String time) throws SQLException {
    ...
    public int getAvailableNurseId() throws SQLException {
        String sql = "SELECT id FROM staff_support WHERE designation='Nurse' LIMIT 1";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ResultSet rs = stmt.executeQuery();
    ...
    public void bookAppointment(Appointment appt) throws SQLException {
        String sql = "INSERT INTO appointment (patient_id, doctor_id, nurse_id, date, time, mode) VALUES (?, ?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, appt.getPatientId());
            stmt.setInt(2, appt.getDoctorId());
            stmt.setInt(3, appt.getNurseId());
            stmt.setString(4, appt.getDate());
            stmt.setString(5, appt.getTime());
            stmt.setString(6, appt.getMode());
            stmt.executeUpdate();
        }
    }
}
